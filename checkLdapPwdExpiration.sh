...
# eg: ldapi://
#
MY_LDAP_HOSTURI="ldap://10.0.3.14:389"
...
#
MY_LDAP_ROOTDN="cn=Manager,dc=volcano-force,dc=com"

#
# LDAP root password (optional)
#
MY_LDAP_ROOTPW="admin"
...
#
MY_LDAP_DEFAULTPWDPOLICYDN="cn=default,ou=policies,dc=volcano-force,dc=com"

#
# LDAP search base for users
# eg: ou=People,dc=example,dc=com
#
MY_LDAP_SEARCHBASE="ou=People,dc=volcano-force,dc=com"
...
MY_MAIL_BODY="
	...
	您的ldap密码将在 %expireTimeTZ 过期。
	密码有效期剩余%expireDays天。
	密码过期后将不能访问SVN、Jira、VPN。
	请尽快修改您的密码。
	修改密码链接：http://ssp.super-chameleon.com:9090
	如密码有效期剩余为0天请通过以下链接修改密码：
	http://ssp.super-chameleon.com:9090/index.php?action=sendtoken
	"

#
# Mail subject
#
MY_MAIL_SUBJECT="您的LDAP账号密码即将过期"
...
do
	...
	${MY_LDAP_SEARCHBIN} ${ldap_param} -s base -b "${dn}" \
		${MY_LDAP_NAME_ATTR} ${MY_LDAP_LOGIN_ATTR} ${MY_LDAP_MAIL_ATTR} ${MY_LDAP_MOBILE_ATTR} pwdChangedTime pwdPolicySubentry \
		> ${buffer_file}
	...
	mail=`grep -w "${MY_LDAP_MAIL_ATTR}:" ${buffer_file} | cut -d : -f 2 \
		| sed "s/^ *//;s/ *$//"`
	mobile=`grep -w "${MY_LDAP_MOBILE_ATTR}:" ${buffer_file} | cut -d : -f 2 \
		| sed "s/^ *//;s/ *$//"`
	...
	expireTimeTZ=`date -d @$expireTime "+%c"`
  ...
  if [ -z "${mail}" ];then
	...
	then
		...
		if [ ${diffTime} -gt ${pwdMaxAge} ]; then
			...
			#python3 dingtalk.py "${mobile}" "${login}" "${expireTimeTZ}" "${expireDays}"
